...
for data in forward_data_lst:
    for k in range(len(forward_outs)):
        if forward_dec_outs[k]:
            ...
            lists_fst=out_folder+'/exp_files/forward_'+data+'_ep*_ck*_'+'fst.lst'
            out_dec_folder=out_folder+'/decode_'+data+'_'+forward_outs[k]
                
            if not(os.path.exists(info_file)):
                
                # Run the decoder
                cmd_decode=cmd+config['decoding']['decoding_script_folder'] +'/'+ config['decoding']['decoding_script']+ ' '+os.path.abspath(config_dec_file)+' '+ out_dec_folder + ' \"'+ files_dec + '\"' + ' \"'+ lists_fst + '\"' 
