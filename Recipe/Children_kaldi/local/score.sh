...
cat $data/text | sed 's:<NOISE>::g' | sed 's:<SPOKEN_NOISE>::g' | sed 's:<UNK>::g' | \
  sed 's:#::g' | sed -r "s:\b(UH|OH|OOH)\b::g" > $dir/scoring/test_filt.txt
...
for wip in $(echo $word_ins_penalty | sed 's/,/ /g'); do
  $cmd LMWT=$min_lmwt:$max_lmwt $dir/scoring/log/score.LMWT.$wip.log \
    ...
    utils/int2sym.pl -f 2- $symtab \| sed \"s:\<UNK\>::g\" \| sed \"s:#::g\" \| sed -r \"s:\\b\(UH\|OH\|OOH\)\\b::g\" \| \
