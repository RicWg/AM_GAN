...
mkdir -p $dir/log
[[ -d $sdata && $data/fst.scp -ot $sdata ]] || split_data.sh $data $nj || exit 1;
...
for f in $sdata/1/feats.scp $sdata/1/cmvn.scp $model $graphdir/HCLG.fst; do
  [ ! -e $f ] && echo "$0: Error: no such file $f" && exit 1;
done
...
if [ $stage -le 0 ]; then
  ...
  $cmd --num-threads $num_threads JOB=1:$nj $dir/log/decode.JOB.log \
    ...
    $model scp:$sdata/JOB/fst.scp "$feats" "ark:|gzip -c > $dir/lat.JOB.gz" || exit 1;
fi
