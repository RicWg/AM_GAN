...
def create_lists(config):
    ...
    for dataset in forward_data_name:
        ...
        [_,lab_data_folder,_]=parse_lab_field2(config[cfg_item2sec(config,'data_name',dataset)]['lab'])
        list_fst=lab_data_folder[0]+'/fst.scp'

        full_list=[line.rstrip('\n')+',' for line in open(list_fst)]
        full_list=sorted(full_list)
          
        full_list_fst_conc=full_list
            
        forward_chunks_fst=list(split_chunks(full_list_fst_conc,N_chunks))

        
        for ck in range(N_chunks):
                
            forward_chunks_fst_split=[];
            for snt in forward_chunks_fst[ck]:
                #print(snt.split(',')[0])
                forward_chunks_fst_split.append(snt.split(',')[0])
                
            output_lst_file=out_folder+'/exp_files/forward_'+dataset+'_ep'+format(ep, N_ep_str_format)+'_ck'+format(ck,N_ck_str_format)+'_'+'fst.lst'
            f=open(output_lst_file,'w')
            forward_chunks_fst_wr=map(lambda x:x+'\n', forward_chunks_fst_split)
            f.writelines(forward_chunks_fst_wr)
            f.close()  
...
def parse_lab_field2(lab):
    
    # Adding the required fields into a list (for decoding, thus only one item for each field)
    lab_count_file=[]
    lab_data_folder=[]
    lab_graph=[]
    ...
    for line in lab.split('\n'):
        ...
        if 'lab_count_file=' in line:
           lab_count_file.append(line.split('=')[1]) 
           
        if 'lab_data_folder=' in line:
            lab_data_folder.append(line.split('=')[1]) 
            
        if 'lab_graph=' in line:
            lab_graph.append(line.split('=')[1]) 
                         
    return [lab_count_file,lab_data_folder,lab_graph]
