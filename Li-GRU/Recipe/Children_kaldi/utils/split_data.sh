...
if [ ! -d $s1 ]; then
  ...
  need_to_split=false
  for f in utt2spk spk2utt spk2warp feats.scp fst.scp text wav.scp cmvn.scp spk2gender \
...
for f in feats.scp fst.scp text vad.scp utt2lang $maybe_wav_scp utt2dur utt2num_frames; do
  if [ -f $data/$f ]; then
